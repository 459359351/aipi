...
_VALID_DIFFICULTIES = {"简单", "一般", "困难"}

_DIFFICULTY_RUBRIC = (
    "\n\n## 难度要求\n"
    "本次所有题目难度应为【{difficulty}】。难度判定参考：\n"
    "- 简单：直接涉及基础概念、原文定义、明确数字与职责，答题只需记忆或原文识别\n"
    "- 一般：需要推导、结合两个或以上知识点，或理解条款的具体应用场景\n"
    "- 困难：涉及跨章节比较、综合判断、实务案例推断或对易混淆规定的辨析"
)


def _difficulty_clause(difficulty: Optional[str]) -> str:
    """生成要插入到 prompt 的难度条款；非法值抛异常，空值返回空串。"""
    if difficulty is None:
        return ""
    if difficulty not in _VALID_DIFFICULTIES:
        raise ValueError(
            f"difficulty 必须是 {sorted(_VALID_DIFFICULTIES)} 之一，当前: {difficulty!r}"
        )
    return _DIFFICULTY_RUBRIC.format(difficulty=difficulty)
...
def generate_single_choices(
    ...
    prompt += _difficulty_clause(difficulty)
    ...
    logger.info(f"[单选题] 解析成功 {len(valid)}/{len(items)} 道 difficulty={difficulty}")
    return valid
...
def generate_multiple_choices(
    ...
    required = {"question_text", "option_a", "option_b", "option_c", "option_d", "correct_answer"}
    valid = [item for item in items if required.issubset(item.keys())]
    logger.info(f"[多选题] 解析成功 {len(valid)}/{len(items)} 道 difficulty={difficulty}")
    return valid
...
def generate_judges(
    ...
    valid = [item for item in items if required.issubset(item.keys())]
    logger.info(f"[判断题] 解析成功 {len(valid)}/{len(items)} 道 difficulty={difficulty}")
    return valid
...
def generate_essays(
    ...
    for item in items:
        ...
        if isinstance(item["scoring_rule"], list):
            ...
            item["scoring_rule"] = json.dumps(item["scoring_rule"], ensure_ascii=False)
        valid.append(item)
    logger.info(f"[简答题] 解析成功 {len(valid)}/{len(items)} 道 difficulty={difficulty}")
    return valid
