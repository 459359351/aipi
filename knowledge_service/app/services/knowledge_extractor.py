...
QA_TAGS_WITH_CANDIDATES_PROMPT_TEMPLATE = """请从下列题目内容中，基于提供的候选标签列表，挑选至少 3 个、最多 5 个最相关的标签。
按相关度从高到低排列，最相关的排在最前面。
必须从候选标签中选择，不要创建候选列表之外的新标签。
如果候选标签总数不足 3 个，则全部选择。
候选标签：{candidate_tags}
只输出 JSON：{{"tags":["最相关标签","次相关标签","第三相关标签"]}}
不要解释，不要多余字段。
题目内容：
{qa_text}
"""
...
def extract_tags_from_qa_direct(
    qa_text: str,
    max_tags: int = 5,
    candidate_tags: Optional[List[str]] = None,
) -> List[str]:
    """从单道题目（题干+答案/解析）直接提炼推荐标签（<=max_tags）。
    当提供 candidate_tags 时，AI 只从候选列表中挑选 3~5 个最相关标签。
    """
    qa_text = (qa_text or "").strip()
    if not qa_text:
        return []
    ...
    if candidate_tags:
        prompt = QA_TAGS_WITH_CANDIDATES_PROMPT_TEMPLATE.format(
            qa_text=qa_text,
